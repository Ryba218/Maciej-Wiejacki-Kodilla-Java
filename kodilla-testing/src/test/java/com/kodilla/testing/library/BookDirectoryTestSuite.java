...
public class BookDirectoryTestSuite {
    ...
    @Test
    public void testListOfBooksInHandOfZero(){
        //Given
        LibraryDatabase libraryDatabaseMock = mock(LibraryDatabase.class);
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);
        List<Book> resultListOfZeroBooks = new ArrayList<>();
        LibraryUser libraryUser = new LibraryUser("Maćko","Zbogdańća","01010199999");

        when(libraryDatabaseMock.listBooksInHandsOf(libraryUser)).thenReturn(resultListOfZeroBooks);

        //When
        List<Book> listBorrowedBooks = bookLibrary.listBooksInHandsOf(libraryUser);
        //Then
        assertEquals(0, listBorrowedBooks.size());
        verify(libraryDatabaseMock,times(1)).listBooksInHandsOf(libraryUser);
    }

    @Test
    public void testListOfBooksInHandOfOne() {
        //Given
        LibraryDatabase libraryDatabaseMock = mock(LibraryDatabase.class);
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);
        ...
        List<Book> listBorrowedBooks = bookLibrary.listBooksInHandsOf(libraryUser);
        //Then
        assertEquals(1,listBorrowedBooks.size());
        verify(libraryDatabaseMock, times(1)).listBooksInHandsOf(libraryUser);
    }

    @Test
    public void testListOfBooksInHandOfFive() {
        LibraryDatabase libraryDatabaseMock = mock(LibraryDatabase.class);
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);
        List<Book> resultListOfFiveBooks = generateListOfNBooks(5);
        LibraryUser libraryUser = new LibraryUser("Maćko", "Zbogdańca", "01010199999");
        ...
        List<Book> listBorrowedBooks = bookLibrary.listBooksInHandsOf(libraryUser);
        //Then
        assertEquals(5,listBorrowedBooks.size());
        verify(libraryDatabaseMock,times(1)).listBooksInHandsOf(libraryUser);
